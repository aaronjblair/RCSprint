...
- Tracks are data (`src/track/TrackDef.ts`); `OvalTrack` builds the banked oval; `src/track/tracks.ts` `generateCareer()` produces the 15 rounds (night rounds 8/12/15). `Scenery.ts` builds trackside (drivers' stand, light towers, start/finish) plus a **per-round themed backdrop** (`TrackDef.backdrop`: mountains/mesas/forest/plains/city/dunes/badlands) and a large **world floor** under it (the 400m infield only covers ±200m, so distant scenery needs ground out to the horizon — half-buried sphere "hills/dunes" float without it). Career save in `src/career/Career.ts` (localStorage); car setup in `src/car/CarSetup.ts`.
