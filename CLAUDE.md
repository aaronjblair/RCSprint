...
- **Backdrops barely show in the racing/driver-stand POV** — that camera is now high and pitches down into the bowl (it frames the whole oval + infield logo), so the distant horizon silhouettes sit above the frame. They read in the **aerial (C) / attract wide** views; verify backdrop changes there (attract reel via `?round=N`), and keep silhouettes tall/close enough to clear the horizon haze.
