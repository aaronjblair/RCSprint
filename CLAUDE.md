...
- **No MCP browser? Screenshot with headless Chrome on the real GPU:** `chrome.exe --headless=new --enable-gpu --window-size=1600,900 --virtual-time-budget=<ms> --screenshot=<abs.png> http://127.0.0.1:5173/?demo` then `Read` the PNG. Software/SwiftShader is far too slow (hundreds of virtual frames hang); the `?demo` URL skips the intro/menu straight into a race; `--virtual-time-budget` also seeks the attract reel's shot cuts.
